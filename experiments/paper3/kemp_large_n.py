"""
Larger-scale measurement of k_emp = T_Valiant / log_2(N) on the
multi-layer overlay union graph G_union = sum_{l=1..L_layers} G_l,
where each G_l is a random d_0-regular graph.

This is the load-bearing k_emp number used in:
  - paper Theorem 4.1 setup cost
  - crossover_boundary.py (T_setup = k_emp * log_2 N * 3 ms)

Previous measurements (paper Table 5.1) covered N in {225, 625, 1225};
this script extends to larger N to verify the ~0.5 constant holds.

USAGE:
    python kemp_large_n.py                      # run default sweep
    python kemp_large_n.py --n N1 N2 N3 ...     # custom N values
    python kemp_large_n.py --L 4 8 16 32        # custom L_layers
    python kemp_large_n.py --trials 5           # custom trial count
    python kemp_large_n.py --max-runtime 600    # cap runtime per (N, L)

Sequential cost scales as O(L * N^2) for sparse union graphs (BFS per
source per phase). At N=11000 single trial ~ 2 min; at N=30000 ~ 15 min;
at N=100000 ~ 3 hours per trial.
"""
import argparse
...
# Use sparse construction when dense would exceed this many GB
DENSE_MEMORY_THRESHOLD_GB = 4.0


def _trial_worker(N, L_layers, d_0, seed, queue):
    """Worker process: runs a single Valiant trial and reports T via queue.

    Auto-selects dense vs sparse based on memory threshold.
    """
    try:
        np.random.seed(seed)
...
def _trial_with_timeout(N, L_layers, d_0, seed, timeout_s):
    """Run one Valiant trial in a subprocess with a hard timeout. Returns
    dict with status: 'ok' (with T,C,D), 'timeout', or 'err' (with message)."""
    queue = mp.Queue()
...
def empirical_T_Valiant(N, L_layers, d_0=8, n_trials=3, seed=42,
                          ...
                          verbose=False):
    """Median Valiant routing depth on the union of L_layers d_0-regular graphs.

    Each trial runs in a subprocess with per_trial_timeout_s hard timeout
    (default: max_runtime_s / n_trials). Trials that timeout are skipped
    and noted in the output.

    Returns dict with median T, individual T values, elapsed time, and
    counts of completed/timed-out trials.
    """
...
def run_sweep(n_values, L_values, n_trials=3, d_0=8, max_runtime_s=None,
    ...
    """Sweep over (N, L) and report k_emp = T_median / log_2(N)."""
    print("=" * 90)
