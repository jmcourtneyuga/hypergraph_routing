"""Tests for routing_lib.graphs: regularity, vertex counts, hyperedge
structure of the key constructors."""
import numpy as np
import pytest
...
def test_3d_aol_clique_is_2d_grid_plus_extra_hyperedges():
    """build_3d_aol_clique returns an N=n*n adjacency matrix layered on top
    of the 2D torus grid with diagonal + skip hyperedges representing the
    3D AOL model (Paper I §3.5)."""
    n = 4
    A = build_3d_aol_clique(n, r=3)
    assert A.shape[0] == n * n
    np.testing.assert_array_equal(A, A.T)
    # 3D AOL adds edges on top of the torus grid, so it has strictly
    # more nonzeros than the bare 2D torus.
...
# ----- Primality helper (used by algebraic constructors) -----
