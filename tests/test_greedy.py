"""Tests for routing_lib.greedy: grid distances, displacement energy,
and matching helpers (Paper I Section 9)."""
import numpy as np
import pytest
...
def test_greedy_matching_reduces_displacement():
    """If pos has two adjacent vertices swapped vs target, greedy matching
    should pick that swap and reduce displacement energy to zero."""
    n = 4
    N = n * n
    A = _path_graph_adjacency(N)
    target = list(range(N))
