#!/usr/bin/env python3
"""
Greedy-stall test extended to N=256.

Verifies the empirical claim of Theorem 9.4 (greedy stall) at the
largest N quoted in the paper appendix; closes a gap in dynamic_adaptive.py
which originally only ran up to n=12 (N=144).
"""
...
def test_greedy_stall_phi_ratio(n, phi_ratio_bound):
    """Theorem 9.4: Phi_stall/Phi_0 converges to ~0.17 for N >= 64.

    We use a loose bound (0.5) to absorb sampling noise; the paper's
    claim is the asymptotic ratio ~0.17.
    """
    N = n * n
    d = max(2, min(8, N - 1))
    if d % 2 == 1:
